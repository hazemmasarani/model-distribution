...
class MambaMixerGate(nn.Module):
    """
    Compute ∆, A, B, C, and D the state space parameters and compute the `contextualized_states`.
    A, D are input independent (see Mamba paper [1] Section 3.5.2 "Interpretation of A" for why A isn't selective)
    ∆, B, C are input-dependent (this is a key difference between Mamba and the linear time invariant S4,
...
class MambaBlock(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx, MixerClass=MambaMixer):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.residual_in_fp32 = config.residual_in_fp32
        self.norm = MambaRMSNorm(config.hidden_size, eps=config.layer_norm_epsilon)
        self.mixer = MixerClass(config, layer_idx=layer_idx)
...
class MambaModel(MambaPreTrainedModel):
    def __init__(self, config, devices: list[torch.device] | None = None):
        super().__init__(config)

        self.devices = devices if devices is not None else [torch.device("cuda:0")]
        self.world_size = len(devices) if devices is not None else 1
