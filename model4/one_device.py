...
class MambaRunFactory:
    def __init__(self, config, devices):
        self.config = config
        self.devices = devices
    
    def create_runner(self, type = "one_device"):
        if type == "one_device":
            return MambaRunOneDevice(self.config, self.devices)
        else:
            raise ValueError(f"Unknown runner type: {type}")

class MambaRunOneDevice:
    def __init__(self, config, devices):
        self.config = config
        self.device = devices[0] if devices else None
        self.layers = nn.ModuleList([MambaBlock(config, layer_idx=idx) for idx in range(config.num_hidden_layers)])
    
    def forward(self, inputs_embeds, cache_params, output_hidden_states, cache_position, attention_mask):
...
class MambaModel(MambaPreTrainedModel):
    def __init__(self, config, divices=None):
        super().__init__(config)

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size)
        factory = MambaRunFactory(config, devices=divices)
        self.mamba_runner = factory.create_runner("one_device")
        # self.layers = nn.ModuleList([MambaBlock(config, layer_idx=idx) for idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, all_hidden_states = self.mamba_runner.forward(
            inputs_embeds, cache_params=cache_params, cache_position=cache_position, attention_mask=attention_mask
        )
        # hidden_states = inputs_embeds
        # all_hidden_states = () if output_hidden_states else None
        # for mixer_block in self.layers:
...
class MambaForCausalLM(MambaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, devices=None):
        super().__init__(config)
        self.backbone = MambaModel(config, devices=devices)
