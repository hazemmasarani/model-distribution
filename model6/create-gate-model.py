...
for k in ckpt_keys:
    ...
    elif "mixer.out_proj.weight" in k or "mixer" not in k:
        new_state_dict[k] = state_dict[k]

    # if "mixer" not in k or "in_proj" in k or "out_proj" in k:
    #     new_state_dict[k] = state_dict[k]
...
model = MambaForCausalLM_Gate(config)
...
model.push_to_hub(
    repo_id="hmasarani/mamba-gate",  # new repo
    private=False
)