...
class MambaMixer(nn.Module):
    ...
    def slow_forward(self, input_states, cache_params: MambaCache_SSM | None=None, cache_position:torch.LongTensor | None=None, attention_mask: torch.LongTensor | None = None):
        ...
        torch.save(input_states, f"../model6/layers_output/ssm_mamba/input_states/input_states_layer_{self.layer_idx}.pt")
        # 1. Gated MLP's linear projection
        # projected_states = self.in_proj(input_states).transpose(1, 2)                   # [batch, 2 * intermediate_size, seq_len]
        # hidden_states, gate = projected_states.chunk(2, dim=1)
        # gate = self.act(gate)  
        hidden_states = self.in_proj(input_states).transpose(1, 2)                                                         # [batch, intermediate_size, seq_len]
        torch.save(hidden_states, f"../model6/layers_output/ssm_mamba/hidden_states/hidden_states_layer_{self.layer_idx}.pt")
        ...
        if cache_params is not None:
            ...
            hidden_states = self.act(self.conv1d(hidden_states)[..., :seq_len])         # [batch, intermediate_size, seq_len]
        torch.save(hidden_states, f"../model6/layers_output/ssm_mamba/hidden_states_conv/hidden_states_conv_layer_{self.layer_idx}.pt")
        ...
        if self.use_mambapy and self.training and cache_params is None:
            ...
            # Save scan_out tensor of layer 0
            if self.layer_idx == 0:
                torch.save(scan_output, "ssm_model_scan_output.pt")
            ...
            torch.save(gate, f"../model6/layers_output/ssm_mamba/gate/gate_layer_{self.layer_idx}.pt")
            torch.save(scan_output, f"../model6/layers_output/ssm_mamba/scan_output_before_gate_mul/scan_output_before_gate_mul_layer_{self.layer_idx}.pt")
            scan_output = scan_output * gate
            torch.save(scan_output, f"../model6/layers_output/ssm_mamba/scan_output_after_gate_mul/scan_output_after_gate_mul_layer_{self.layer_idx}.pt")
        else:
            scan_outputs = []
        ...
        torch.save(contextualized_states, f"../model6/layers_output/ssm_mamba/contextualized_states/contextualized_states_layer_{self.layer_idx}.pt")
...
class MambaBlock(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        torch.save(hidden_states, f"../model6/layers_output/ssm_mamba/hidden_states_rsqrt/hidden_states_rsqrt_layer_{self.layer_idx}.pt")
...
class MambaModel_SSM(MambaPreTrainedModel_SSM):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        torch.save(input_ids, "../model6/layers_output/ssm_mamba/forward_model_input_ids/forward_model_input_ids_layer_0.pt")
        ...
        torch.save(inputs_embeds, "../model6/layers_output/ssm_mamba/inputs_embeds/inputs_embeds_layer_0.pt")
